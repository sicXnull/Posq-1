...
public:
    ...
    bool WriteZPOSQSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPOSQSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPOSQSeed_deprecated(uint256& seed);
    bool EraseZPOSQSeed();
    bool EraseZPOSQSeed_deprecated();

    bool WriteZPOSQCount(const uint32_t& nCount);
    bool ReadZPOSQCount(uint32_t& nCount);
