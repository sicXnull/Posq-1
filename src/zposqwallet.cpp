...
CzPOSQWallet::CzPOSQWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zposq seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPOSQSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPOSQSeed_deprecated()) {
                    LogPrintf("%s: Updated zPOSQ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zposq seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zposq wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPOSQWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPOSQCount(nCountLastUsed);
    else if (!walletdb.ReadZPOSQCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPOSQWallet::Lock()
{
    seedMaster = 0;
}

void CzPOSQWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPOSQWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPOSQ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPOSQWallet::LoadMintPoolFromDB()
{
...
void CzPOSQWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPOSQWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPOSQWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zposqTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPOSQWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPOSQ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zposqTracker which also adds to database
    pwalletMain->zposqTracker->Add(dMint, true);
...
void CzPOSQWallet::SeedToZPOSQ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPOSQWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPOSQWallet::GenerateDeterministicZPOSQ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPOSQWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPOSQWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
