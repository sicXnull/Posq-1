...
#ifndef POSQ_ZPOSQWALLET_H
#define POSQ_ZPOSQWALLET_H
...
public:
    CzPOSQWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZPOSQ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZPOSQ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //POSQ_ZPOSQWALLET_H
