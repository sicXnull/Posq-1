...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zposqs that have been in mempool long
                    //and higher priority to zposqs that are large in value
                    ...
                    // zPOSQ spends can have very large priority, use non-overflowing safe functions
        ...
        if (fZerocoinActive) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zPOSQ will be disabled and previous block's checkpoint is all that will be needed
